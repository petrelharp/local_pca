...
samples$ID[ match(gsub("-I","",wierd.names),wierd.names) ] <- wierd.names
...
chr1.mds <- cmdscale( chr1.pcdist, eig=TRUE, k=4 )
colnames(chr1.mds$points) <- paste("MDS coordinate", 1:ncol(chr1.mds$points))
```
...
pairs( chr1.mds$points, pch=20, col=adjustcolor(chrom.cols,0.75) )
```

Now let's extract the extreme windows in the MDS plot:
```{r get_corners}
mincirc <- enclosing_circle( chr1.mds$points[,1:2] )
corners <- corners( chr1.mds$points[,1:2], prop=.05 )
corner.cols <- c("red","blue","purple")
ccols <- rep("black",nrow(chr1.mds$points))
for (k in 1:ncol(corners)) {
    ccols[ corners[,k] ] <- corner.cols[k]
}
plot( chr1.mds$points[,1:2], pch=20, col=adjustcolor(ccols,0.75), 
...
... and look at where these lie along the chromosomes
```{r show_corners}
for (k in 1:ncol(chr1.mds$points)) {
    plot( win.mids/1e6, chr1.mds$points[,k], pch=20, 
