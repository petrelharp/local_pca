...
```
```{r plot_corners, fig.width=3*fig.dim}
...
(warning: takes awhile and a lot of RAM)
```{r corner_pca, cache=TRUE}
corner.npc <- 4
corner.regions <- lapply( 1:ncol(mds.corners), function (k) {
            regions[ mds.corners[,k], ]
    } )
corner.pca <- lapply( corner.regions, function (reg) {
            geno <- vcf_multi_query( chrom.list=chroms, file=bcf.files, regions=reg )
            cov_pca(geno,k=corner.npc)
    } )
```

First, here are the first `r corner.npc` PCs for each corner, together:
```{r plot_corner_pca, fig.width=3*fig.dim}
for (k in seq_along(mds.corners)) {
    vectors <- matrix( corner.pca[[k]][-(1:(1+corner.npc))], ncol=corner.npc )
    colnames(vectors) <- paste("PC", 1:corner.npc)
    pairs(vectors)
}
```
