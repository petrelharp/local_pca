...
```{r get_corners}
mds.corners <- corners( mds.coords[,mds.cols[1:2],drop=FALSE], prop=.05 )
corner.coords <- t( apply( mds.corners, 2, function (k) { colMeans( mds.coords[k,mds.cols,drop=FALSE], na.rm=TRUE ) } ) )
    ...
    for (j in (i+1):corner.npc) {
        for (k in 1:ncol(mds.corners)) {
            ...
            plot(vectors, pch=pop.pch[samps$Country.of.Origin], 
                    col=pop.cols[samps$Country.of.Origin], cex.axis=1.5 )
            if (i==1 && j==2) {
                mtext(paste("corner",k),side=3,cex=2,col=corner.cols[k])
            }
        }
    }
}
