...
\reply{
    Good point: we now reference this work \refref.
}
...
\reply{
    This is a good point, but XXX
    We now cite PCAdmix \revreffull{1}{8}.
}
...
\reply{
    ...
    and obtained the best current version of the Medicago recombination map
    ...
    Both are on a previous version of the assembly, 
    and the LDhat estimates from only three chromosomes
    have been published.
    Given uncertainties in using and interpreting these data,
    and the further uncertainty in what the relevant window size should be
    (strength of background selection depends on ``nearby'' density of sites under selection,
    but how close ``nearby'' is depends on many factors),
    we decided not to carry though this analysis.
}
