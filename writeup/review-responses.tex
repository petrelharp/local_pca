...
We appreciate the thoughtful comments by the reviewers and the editors,
and have made changes to address all of the points raised.
The changes are mostly to what is discussed;
we sympathize with the additional testing suggested by Reviewer 1,
but think that it falls outside the scope of this paper,
and would be appropriate for follow-up work that builds on our descriptive method
to create more specific, hypothesis-driven tools.

We have also included a color-coded manuscript highlighting differences 
between this version and the last one (but the line numbers referred to in the 
``response to reviewers'' refer to the standard manuscript).
The file is named ``\url{local_pca_paper-diff164c6.pdf}'',
and is labeled another ``response to reviewers'' for lack of a better category in the submission system.
...
\reply{
    ...
    However, we do not set out to make such a method,
    but rather a visualization method in which, empirically, inversions often leave a large signal.
    We are not sure if it would be productive to turn this into a quantitative method, either:
    as we have further clarified in the paper \llname{ll:inversion_caveat},
...
\reply{
    ...
    although a region of low recombination could be dominated by a single tree,
    ...
    as we see here.
    We've tried to make this argument more clear. {\llname{ll:not_neutral}}
    ...
    % (see for instance \citet{jensen2014unfounded,harris2018unfounded} and subsequent rejoinders).
    We've added more to the discussion on this. \revref
}

\begin{point}{}
    The authors refer to supplementary Figures sometimes as ``Figure S6'' and sometimes as ``Supplementary Figure S6''. 
    In certain journals, these two could denote different figures so I would select one way only.
\end{point}
...
\reply{
    Apologies for not realizing the precise nature of the original concern.
    ...
    spurious structure created by this problem would dissappear when using a larger number of PCs
    -- this does not happen with our empirical results.
    We have added discussion of these points. {\revref}
    ...
    and so might help prevent sensitivity to PC switching.
}
