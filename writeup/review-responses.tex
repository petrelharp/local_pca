...
\reply{
    ...
    For this reason, varying recombination rate in simulations,
    and varying window size and type in analysis
    suffices to check for the effects of both recombination and mutation rate.
    ...
    The point is addressed by comparing results with windows of different types --
    windows of equal length in bp (or in SNPs) have different lengths in cM;
    since these different choices show nearly identical patterns in all cases we have examined,
    recombination rate variation cannot be driving the results
    (e.g., compare Figures \ref{fig:mds_allchr} and Supplemental Figure \plr{XXX}).
}
...
\reply{
    We have added a representative figure (for the Drosophila data, $k=5$),
...
\reply{
    We have added this: see supplemental figure \ref{sfig:mds_medicago_allchr_1e3}. \revref
}
...
\reply{
    Again, we have added this: see supplemental figure \plr{XXX}. \revref
}
...
\reply{
    ...
    whereby the amount of missingness does not vary by population \llname{ll:test_missing}.
    ...
    but if missingness is in fact correlated with changes in patterns of relatedness (as in these cases),
    ...
    and mentioned in the text \revref,
...
\reply{
    ...
    but for this audience would be getting too far down in the weeds,
    and including the requested simulations would inflate an already lengthy paper.
    Additionally, we aim to provide a good method, not to prove that the method
...
\reply{
    We have included the plots for Drosphila data using $k=5$:
    see Figure \ref{fig:dmel_k5},
    and hope that the readers will take our word on this point more generally.
}
