...
and check that results hold using more. \revpoint{1}{7}
...
does not depend on PC ordering.) \linelabel{ll:another_pc_point}
...
which is invariant under ordering of the PCs \revpoint{1}{3},
...
Details are given in Appendix \ref{apx:gaussian_sims}.

To provide a realistic test,
we next used forwards-time, individual-based simulations, implemented using SLiM v3 \citep{slim_v3},
which are described in detail in Appendix \ref{apx:slim_sims}.
To provide realistic population structure for PCA to identify,
each simulation ahd at least 5,000 diploid individuals,
...
To improve speed, we used tskit \citep{tskit_paper} to record tree sequences
and add neutral mutations afterwards, at a rate of $10^{-9}$ per bp per generation.
...
(Supplemental Figure \ref{fig:test_missing}).
...
The same type of simulations with dramatically different sample sizes
showed that using weighted PCA substantially improved identification of different genomic regions
(Supplemental Figure \ref{fig:test_weighted_pca}).

Individual-based simulations using SLiM \citep{slim_v3} allowed us to test the effects
of recombination and mutation rate variation, as well as linked selection.
Results are shown in Supplemental Figures \ref{sfig:slim_sims1} and \ref{sfig:slim_sims2}.
...
we used the same simulations to show that variation in mutation rate similarly has no effect.
On the other hand,
a recombination map with hotspots
(the HapMap human female map for chromosome 7 \citep{hapmap})
induced outliers at long regions of low recombination rate
(also as expected).

Simulations with linked selection produced mixed results
(Figure \ref{sfig:linked_selection}).
...
leading to a stronger effect of linked seleciton.
These tests are not meant to be comprehensive survey of linked selection,
but only to demonstrate that linked selection can produce signals simlar to what we see in real data.
...
\paragraph{Testing}
To demonstrate the utilitiy of this method XXX
see Figure \ref{fig:test_weighted_pca}.
...
\begin{figure}
    ...
    \end{center}
    \caption{
        PCA plots for regions colored in \Figure \ref{sfig:mds_run_015598}
        \label{sfig:pca_corner_run_015598}
    }
\end{figure}

\begin{figure}
    \begin{center}
       \includegraphics[width=0.82\textwidth]{FigS_plot_corners-1_run_005464}
    \end{center}
    \caption{
        MDS visualizations of simulation run 005464 (RECOMBTYPE, ``base"; SELTYPE, ``selected")
        \label{sfig:mds_run_005464}
    }
\end{figure}

\begin{figure}
    \begin{center}
       \includegraphics[width=0.82\textwidth]{FigS_plot_corners-1_run_031486}
    \end{center}
    \caption{
        \plr{KEEP}
        MDS visualizations of simulation run 031486 (RECOMBTYPE, ``base"; SELTYPE, ``selected")
        \label{sfig:mds_run_031486}
    }
\end{figure}

\begin{figure}
    \begin{center}
       \includegraphics[width=0.82\textwidth]{FigS_plot_corners-1_run_009673}
    \end{center}
    \caption{
        \plr{KEEP}
        MDS visualizations of simulation run 009673 (RECOMBTYPE, ``step"; SELTYPE, ``selected")
        \label{sfig:mds_run_009673}
    }
\end{figure}

\begin{figure}
    \begin{center}
       \includegraphics[width=0.82\textwidth]{FigS_plot_corners-1_run_016913}
    \end{center}
    \caption{
        MDS visualizations of simulation run 009673 (RECOMBTYPE, ``base"; SELTYPE, ``balancing")
        \label{sfig:mds_run_016913}
    }
\end{figure}
...
\begin{figure}
    ...
    \end{center}
    \caption{
        PCA plots for regions colored in \Figure \ref{sfig:mds_run_016913}
        \label{sfig:pca_corner_run_016913}
    }
\end{figure}
...
\begin{figure}
    ...
    \end{center}
    \caption{
        The missingness ratio along genome and the correlation of MDS against missingness in Drosophila.
...
\begin{figure}
    ...
    \end{center}
    \caption{
        From simple example -- base case; and with 50\% missing data.
    } \label{fig:test_missing}.
\end{figure}

\begin{figure}
    \begin{center}
        \includegraphics[width=0.82\textwidth]{simple_example_sample_diffs}
        \includegraphics[width=0.82\textwidth]{simple_example_reweighted}
    \end{center}
    \caption{
        From simple example -- different sample sizes; unweighted (top); weighted by $1/\sqrt{n_i}$ (bottom).
    } \label{fig:test_weighted_pca}.
\end{figure}
