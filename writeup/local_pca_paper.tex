...
\begin{align} \label{eqn:weighted_objective}
    \| W^{1/2} (C - U \Lambda U^T) W^{1/2} \|^2 = \sum_{ij} \sqrt{W_i W_j} \left( G_{ij} - \sum_m \lambda_{m} U_{im} U_{jm} \right)^2 ,
\end{align}
...
so if we define $U = W^{-1/2} V$ then 
...
The proportion of variance explained is calculated exactly as before,
but the variance is computed in $\ell_2(\sqrt{w})$:
\begin{align*}
    \frac{ \lambda_m^2 }{ \sum_\ell \lambda_\ell^2 } = \frac{ \sum_{ij} ( \lambda_m \sqrt{w_i w_j} U_{im} U_{jm} )^2 }{ \sum_{ij} \sqrt{w_i w_j} C_{ij}^2 } .
\end{align*}
