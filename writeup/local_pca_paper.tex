...
\citep[sometimes dramatic:][]{langley2012genomic};
...
We also used whole-genome sequencing data 
from the Drosophila Genome Nexus \citep[\url{http://www.johnpool.net/genomes.html}][]{lack2015drosophila},
consisting of the Drosophila Population Genomics Project phases 1--3 \citep[DPGP][]{langley2012genomic,pool2012population},
and additional African genomes \citep[AGES][]{lack2015drosophila},
...
\begin{figure}
    ...
    \caption{
        ...
        In all plots, each point represents one window along the genome.
         ...
         Solid black lines are for the inversions we used to obtaining Figure \ref{fig:pca_by_inversion},
...
these three groups of windows are shown in color in Figure \ref{fig:mds_allchr} and PCA plots for each collection are shown in Figure \ref{fig:pca_by_pop}.
...
Two more inversions, In(3L)P and In(3R)Mo, are at very low frequency in these samples, 
...
with results shown in Supplementary Figure \ref{fig:mds12_chr1_8_human}, \ref{fig:mds12_chr9_16_human}, \ref{fig:mds12_chr17_22_human}.
...
These may be primarily inversions:
for instance, the eleven windows that are outliers in the first MDS coordinate of chromosome 8 (Figure \ref{fig:mds_human}) 
coincide with the position of a particularly large, polymorphic inversion on 8p23. 
More generally, we see signals coincident with 
...
PCA plots of these windows show a characteristic trimodal shape (Figure \ref{fig:pca_chr8_outliers}),
...
a signal that has been proposed as a method to identify inversions \citep{ma2012investigation}.
However, distinguishing this hypothesis from others,
...
the two types of population structure with higher gene density (colored green and purple in Figure \ref{fig:mds12_medicago}
...
we computed gene density for each window using gene models in Mt4.0 JBrowse \citep{tang2014improved}. 
we downloaded the gene model track from Mt4.0 JBrowse at jcvi.org,
extracted the start and end positions of each predicted mRNA transcript,
counted how many of these fell in each window, 
and use the mean number of the starts and ends in a window to represent the gene count for that window.
The endpoints of the windows were defined to be halfway between the last SNP of each window and the first SNP in the next window.
...
\begin{figure}
    \begin{center}
       \includegraphics[width=0.9\textwidth]{Fig5_MDS_and_gene_count_allchr_update}
    \end{center}
    \caption{
...
so this seems unlikely to give different results.
...
Our software allows different choices for how many PCs to use in approximating structure of each window ($k$ in equation \ref{eqn:est_cov}),
...
It would be interesting to see if somehow removing the effects of inversions in the \textit{Drosophila melanogaster} or human datasets
...
\begin{figure}
    \begin{center}
       \includegraphics[width=0.9\textwidth]{FigS_PCA_chr8_outliers_combine}
    \end{center}
    \caption{
