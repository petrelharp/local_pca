...
which is invariant under ordering of the PCs \revpoint{1}{2},
...
and added neutral mutations afterwards, at a rate of $10^{-9}$ per bp per generation.
...
(also as expected). \linelabel{ll:hotspot_recomb}
...
(see \Figure \ref{fig:dmel_k5} for the same plot with $k=5$ PCs), \revpoint{1}{3}
and are not driven by variation in missingness (see \Figure \ref{sfig:dpgp_missingness}). \revpoint{2}{4}
...
and number of PCs (Table \ref{tab:param_cors}).
\revpoint{1}{4} \revpoint{1}{5} \revpoint{1}{6}
...
the effects of neutral processes are not expected to show such large, chromosome-scale correlations.  \linelabel{ll:not_neutral}
Furthermore, 
...
\citep{charlesworth1997effects,charlesworth2012effects,pease2013accurate,hedrick2013adaptive,burri2015linked,corbettdetig2015natural,harris2016genetic,martin2016natural,phung2016determining}. \revpoint{2}{16}
...
Alternatively, windows could be chosen to have equal length in genetic distance, % or have equal length in physical distance
...
or one constructed using only sites unliked to the focal SNP. \revpoint{2}{9}   %unlike ---> unlink?
...
\caption{
    ...
    (so typical variation is of order of 50\% of the actual values).  %A little confused about this line. What does this 50% mean?
...
produced the first plot in Figure \ref{fig:test_missing}.
...
separate from the other half. \linelabel{ll:pc_switching_test}
...
producing the second plot of Figure \ref{fig:test_missing}. \linelabel{ll:test_missing}
...
and the outer two sevenths has a rage 64 times higher.
...
\begin{figure}
    ...
    \caption{
        ...
        In all plots, each point represents one window along the genome.
         ...
         rows correspond to chromosome arms.
%         Colors are consistent for plots in each row.   %this plot is all black points. 
         Vertical lines show the breakpoints of known polymorphic inversions.   
