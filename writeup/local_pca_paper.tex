...
\begin{table}[ht]
\centering
\begin{tabular}{p{0.8in}p{.8in}p{1in}p{1.2in}p{1.4in}}
  \hline
...
\plr{I'm talking about flipping signs up here, even though in the code we do it when computing variances, since it makes sense conceptually here.}
...
These values are shown calculated separately for each chromosome arm in the \textit{Drosophila} dataset in Table \ref{tab:window_sizes}.
Finally, we choose 100 SNPs, 1000 SNPs and 10000 SNPs as window length for human, \textit{Drosophila}, and \textit{Medicago} respectively.



\begin{table}[ht]
\centering
    \begin{tabular}{rlp{1.1in}rrrrr}
  \hline
...
\caption{
    ...
    \plr{make 'window length' in bigger bar above the values}
...
\begin{figure}
    \begin{center}
       \includegraphics[width=1\textwidth]{Fig1_Together_MDS_plot}
    \end{center}
    \caption{
...
\begin{figure}
    \begin{center}
       \includegraphics[width=0.7\textwidth]{Medicago_MDS_1D_and_gene_count}
    \end{center}
    \caption{
