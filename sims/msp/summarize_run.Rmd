...
if (!is.null(opt$sample_info)) {
    ...
    warning("No populatio information in the sample file, %s.", samp.file)
    samps <- data.frame( 
            ID=sample.ids,
...
```{r get_corners}
mds.corners <- corners( mds.coords[,mds.cols[1:2]], prop=.01 )
...
for (k in seq_along(corner.pca)) {
    corner.pca[[k]] <- cov_pca(covmat=covmat, k=corner.npc, w=opt$weights)
}
```
...
dim(pop.names) <- c(10,10)
pop.cols <- rainbow_hcl(nlevels(samps$population))
# max pch value is 25
# pop.pch <- seq_len(nlevels(samps$population)) %% 25
pop.pch <- rep(c(0:4,14:18),10)
plot( row(pop.names), col(pop.names),
       cex=4, pch=pop.pch, col=pop.cols,
