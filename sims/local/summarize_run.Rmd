...
knitr::knit_hooks$set(slide.mar = function(before, options, envir) {
                if (before) {
                    par(mai=c(.4,.4,.05,.05),  # margin for presentations
                        mgp=c(2,0.7,0))
                }
            })
knitr::opts_chunk$set(slide.mar=TRUE)
pdf_copy <- function (
                width=5, 
...
if (!is.null(opt$sample_info)) {
    ...
    if (all(grepl("^msp_",sample.ids)) & is.numeric(samps$id)) {
        ...
        samps$id <- paste0("msp_", 0:(nrow(samps)-1))
    }
...
spacing <- 1
opar <- par(mar=par("mar"),mgp=c(0.5,0.8,0))
