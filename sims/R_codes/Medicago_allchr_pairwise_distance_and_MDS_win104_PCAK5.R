...
for(i in 1:length(fn))
{
    ...
    eigenstuff <- eigen_windows(coded, win=10^4, k=5,recode=FALSE)
    windist <- pc_dist(eigenstuff, npc=5)
    write.table(windist,file=paste("/home/cmb-12/fs3/hli465/FormatVCFbgz/pairwise_distance_between_win104",fn[i],"PCAk5",sep="_"),sep="\t")
...
data<-read.table(gettextf("pairwise_distance_between_win104_%s_PCAk5",chr),check.names=FALSE)
...
pdf(file="/home/cmb-12/fs3/hli465/FormatVCFbgz/Medicago_MDS_plot_allchr_win104_PCAk5.pdf",width=5,height=7)
