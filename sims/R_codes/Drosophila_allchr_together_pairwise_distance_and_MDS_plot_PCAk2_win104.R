...
for(i in 1:length(fn))
{
   ...
   eigenstuff <- eigen_windows(coded, win=10^4, k=2,recode=FALSE)
   windist <- pc_dist(eigenstuff, npc=2)
   write.table(windist,file=gettextf("/home/cmb-12/fs3/hli465/Dpgp/all_samples_%s/pairwise_distance_between_win104_%s_PCAk2",fn[i],fn[i]),sep="\t")
}

#Chr2L
data<-read.table("/home/cmb-12/fs3/hli465/Dpgp/all_samples_Chr2L/pairwise_distance_between_win104_Chr2L_PCAk2",check.names=FALSE)
...
#Chr2R
data<-read.table("/home/cmb-12/fs3/hli465/Dpgp/all_samples_Chr2R/pairwise_distance_between_win104_Chr2R_PCAk2",check.names=FALSE)
ind1 <- apply(data, 1, function(x) all(is.na(x)))
a1=data[!ind1,]
ind2 <- apply(a1, 2, function(x) all(is.na(x)))
a2=a1[,!ind2]  
...
#Chr3L
data<-read.table("/home/cmb-12/fs3/hli465/Dpgp/all_samples_Chr3L/pairwise_distance_between_win104_Chr3L_PCAk2",check.names=FALSE)
ind1 <- apply(data, 1, function(x) all(is.na(x)))
a1=data[!ind1,]
ind2 <- apply(a1, 2, function(x) all(is.na(x)))
a2=a1[,!ind2] 
...
#Chr3R
data<-read.table("/home/cmb-12/fs3/hli465/Dpgp/all_samples_Chr3R/pairwise_distance_between_win104_Chr3R_PCAk2",check.names=FALSE)
ind1 <- apply(data, 1, function(x) all(is.na(x)))
a1=data[!ind1,]
ind2 <- apply(a1, 2, function(x) all(is.na(x)))
a2=a1[,!ind2]     
...
#ChrX
data<-read.table("/home/cmb-12/fs3/hli465/Dpgp/all_samples_ChrX/pairwise_distance_between_win104_ChrX_PCAk2",check.names=FALSE)
ind1 <- apply(data, 1, function(x) all(is.na(x)))
a1=data[!ind1,]
ind2 <- apply(a1, 2, function(x) all(is.na(x)))
a2=a1[,!ind2] 
...
pdf(file="/home/cmb-12/fs3/hli465/Dpgp/Fig1_allchr_Together_MDS_plot_PCAk2_win104.pdf",width=6,height=6.5)
